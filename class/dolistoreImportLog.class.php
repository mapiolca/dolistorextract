...
class DolistoreImportLog extends CommonObject
{
	...
	public static function add($db, $level, $message, $fkOrder = 0, $source = 'import', $context = array(), $user = null, $fkBatch = 0)
	{
		...
		$sql .= self::quote($db, $source).',';
		$sql .= self::quote($db, $level).',';
		$sql .= self::quote($db, $message).',';
		$sql .= self::quote($db, !empty($context) ? json_encode($context) : '').',';
	 ...
	 * @return string
	 */
	private static function quote($db, $value)
	{
		if ($value === null || $value === '') {
			return 'NULL';
		}
